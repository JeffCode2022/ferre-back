...
const register_admin = async function(req, res) {
    var data = req.body; // Los datos recibidos en el cuerpo de la solicitud

    // Verifica si el email ya está registrado en la base de datos
    var admin_exists = await Admin.findOne({ email: data.email });

    if (admin_exists) {
        return res.status(400).send({ message: 'El correo electrónico ya está registrado', data: undefined });
    }

    try {
        // Encriptamos la contraseña antes de guardarla
        bcrypt.hash(data.password, null, null, async function(error, hashedPassword) {
            if (error) {
                return res.status(500).send({ message: 'Error al encriptar la contraseña', data: undefined });
            }

            // Creamos un nuevo administrador con los datos proporcionados
            ...
            // Retornamos una respuesta exitosa con los datos del administrador y su token JWT
            res.status(200).send({
                ...
                token: jwt.createToken(newAdmin)  // Generamos un token para la autenticación
            });
        });
    } catch (error) {
        res.status(500).send({ message: 'Error al registrar el administrador', data: undefined });
    }
};
